#!/bin/bash
#------------------------------------------------------------------------------
# @description Searches and replaces strings in file contents and filenames
# @description within a specified target path.
# @param STR_TO_SRCH (required) - The string to search for.
# @param STR_TO_REPL (required) - The string to replace with.
# @param TGT_PATH (required) - The target directory path to perform morphing in.
# @example STR_TO_SRCH=old STR_TO_REPL=new TGT_PATH=/path/to/dir ./run -a do_morph_path
#------------------------------------------------------------------------------
do_morph_path() {
  set -x
