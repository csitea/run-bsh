...
# @description  private nda-edw-utl project tree. Wired into every Confluence
# @description  publish action so nda-edw-utl content can NEVER reach the
# @description  Alchemists Confluence space.
# @description
# @description  nda-edw-utl is personal automation / consulting tooling; it is
...
do_require_not_utl_source() {
  ...
  local forbidden="/opt/nda/nda-edw/nda-edw-utl"
  if [[ "$real" == "$forbidden"* ]]; then
    do_log "FATAL Refusing to publish from nda-edw-utl private repo: $path"
    do_log "FATAL  resolved to: $real"
    do_log "FATAL nda-edw-utl is personal automation — never published to Confluence."
