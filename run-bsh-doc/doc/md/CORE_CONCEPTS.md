...
1. **Environment Setup:** Sets up global variables along the canonical path layout `${BASE_PATH}/${ORG}/${APP}/${APP}-${PROJ_KIND}`. Derived globals: `BASE_PATH`, `ORG_PATH`, `APP_PATH`, `PROJ_PATH`, `ORG`, `APP`, `PROJ`, `PROJ_KIND`. Any of `ORG`/`APP`/`PROJ_KIND` can be overridden by exporting the variable before invocation.
...
See [DIRECTORY_STRUCTURE.md](DIRECTORY_STRUCTURE.md) for the building-block table.
