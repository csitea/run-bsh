...
### 3. Path & Identity Discovery 🧭
The framework automatically detects its context from its location in the canonical directory structure: `${BASE_PATH}/${ORG}/${APP}/${PROJ}`.

| Variable | Meaning | Example |
| :--- | :--- | :--- |
| `RUN_UNIT` | The filename of the entry script. | `run-bsh-utl.sh` |
| `BASE_PATH` | The root installation directory. | `/opt` |
| `ORG` | The organization slug. | `csi` |
| `APP` | The application or project suite name. | `run-bsh` |
| `PROJ` | The specific sub-project leaf directory. | `run-bsh-utl` |
| `PROJ_KIND` | The role suffix (derived from `PROJ`). | `utl` |
| `PROJ_PATH` | Absolute path of the current module. | `/opt/csi/run-bsh/run-bsh-utl` |
| `APP_PATH` | Absolute path of the application umbrella. | `/opt/csi/run-bsh` |
| `ORG_PATH` | Absolute path of the organization root. | `/opt/csi` |
| `VAR_DIR` | Sibling `var` root for persistent data. | `/var` |
| `FRW_PATH` | Path to parent `*-frw` project (if detected). | `/opt/csi/run-bsh/run-bsh-frw` |

Any of `ORG`, `APP`, or `PROJ_KIND` can be overridden by exporting the variable before framework invocation.


### 4. Decentralized Parameter Parsing ⌨️
